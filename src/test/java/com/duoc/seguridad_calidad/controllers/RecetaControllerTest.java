...
class RecetaControllerTest {
        ...
        void testHome() throws Exception {
                ...
                when(recetaService.obtenIngredientes(anyString()))
                                .thenReturn(Arrays.asList(ingrediente1, ingrediente2));
        ...
        void testPublicarReceta() throws Exception {
                ...
                when(recetaService.publicarReceta(any(CrearReceta.class), anyString()))
                                .thenReturn(ResponseEntity.ok().build());
                when(recetaService.obtenIngredientes(anyString()))
                                .thenReturn(Arrays.asList(new Ingrediente(1L, "Tomate")));
        ...
        void testComentar() throws Exception {
                ...
                comment.setIdReceta(recetaId);
                comment.setIdUsuario(mockUser.getId());
                comment.setCalificacion(calificacion);

                Receta mockReceta = new Receta();
                mockReceta.setId(recetaId);
                mockReceta.setNombre("Pizza");

                when(tokenStore.getUserModel()).thenReturn(mockUser);
                when(tokenStore.getToken()).thenReturn("mockToken");
                when(recetaService.obtenerComentarios(anyLong())).thenReturn(Arrays.asList(comment));
                when(recetaService.obtenerRecetasByID(anyLong(), anyString())).thenReturn(mockReceta);

                // Act & Assert
                mockMvc.perform(post("/receta/comentar/{id}", recetaId)
                                .param("comentario", comentario)
                                .param("calificacion", String.valueOf(calificacion)))
                                .andExpect(status().isOk())
                                .andExpect(view().name("recetas/verreceta"))
                ...
                verify(tokenStore, times(2)).getUserModel();
                verify(tokenStore, times(3)).getToken();
                verify(recetaService, times(1)).obtenerComentarios(recetaId);
                verify(recetaService, times(1)).obtenerRecetasByID(recetaId, "mockToken");
                verify(recetaService, times(1)).comentarRecetas(any(ComentarioReceta.class), eq("mockToken"));
        }
